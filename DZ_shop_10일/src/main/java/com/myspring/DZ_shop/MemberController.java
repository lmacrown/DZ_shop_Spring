...
public class MemberController {
	...
	public Map<String, Object> send_id(Locale locale, Model model, HttpSession session,
		...
		try {
			...
			/*
			 * Calendar cal = Calendar.getInstance(); cal.add(Calendar.MINUTE, 5); String
			 * mLater = dateFormat.format(cal.getTime());
			 */
	...
	public Map<String, Object> myItemDelete(Locale locale, Model model, HttpSession session,
		...
		logger.info("구매 삭제 : "+ map.get("id"));
		Map<String, Object> result = new HashMap<>();
		try {
			goodsService.myItemDelete(map.get("id"));
			result.put("status", true);
			result.put("url", "/DZ_shop/");
		}catch(Exception e) {
			e.printStackTrace();
