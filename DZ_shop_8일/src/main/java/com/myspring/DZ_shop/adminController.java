...
//		out.println("send Mail");
	...
	@ResponseBody
	public Map<String, Object> postRegister(Model model, @RequestBody HashMap<String, String> map,
			HttpSession session) throws IOException {
		Map<String, Object> result = new HashMap();
		Date date = new Date();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		String formattedDate = dateFormat.format(date);
		logger.info("어드민 게시글 생성 "+map.get("title"));
		try {
	...
	public Map<String, Object> goodsUpdate(Model model, HttpSession session,
			@RequestBody HashMap<String, String> map) throws IOException {
		Map<String, Object> result = new HashMap();
		Date date = new Date();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
